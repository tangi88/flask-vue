...
def get_data_mvideo(url):
    ...
    response_redirect = s.get(redirect_url, headers=headers, cookies=cookies, allow_redirects=True)

    prices = response_redirect.json().get('body').get('materialPrices')
    price = 0
...
@get_price
def get_data_doublesports(soup):
